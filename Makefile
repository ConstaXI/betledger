...
export HTTP_PORT DATABASE_URL

.PHONY: help dev run db-up db-down db-reset migrate-up migrate-down migrate-status \
	test test-race test-integration vet fmt generate check

help: ## List the available targets
	@grep -E '^[a-zA-Z_-]+:.*?## ' $(MAKEFILE_LIST) | awk 'BEGIN {FS = ":.*?## "} {printf "  %-18s %s\n", $$1, $$2}'

dev: db-up migrate-up run ## Start the database, apply migrations and run the application
...
db-reset: ## Stop PostgreSQL and delete its data
