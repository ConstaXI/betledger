...
func TestApplicationStartsServesAndReleasesResources(t *testing.T) {
	...
	application := testenv.StartApplication(t, database.URLWithApplicationName(applicationName), identityProvider, broker)
...
func TestApplicationKeepsStateAcrossRestart(t *testing.T) {
	...
	first := testenv.StartApplication(t, database.URL, identityProvider, broker)
	assert.Equal(t, http.StatusCreated, first.OpenWallet(t, playerID))
	first.Stop(t)

	restarted := testenv.StartApplication(t, database.URL, identityProvider, broker)
