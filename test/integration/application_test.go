...
func TestApplicationStartsServesAndReleasesResources(t *testing.T) {
	...
	application := binary.StartAPI(t, database.URLWithApplicationName(applicationName), identityProvider, broker)
...
func TestApplicationKeepsStateAcrossRestart(t *testing.T) {
	...
	first := binary.StartAPI(t, database.URL, identityProvider, broker)
	assert.Equal(t, http.StatusCreated, first.OpenWallet(t, playerID))
	first.Stop(t)

	restarted := binary.StartAPI(t, database.URL, identityProvider, broker)
