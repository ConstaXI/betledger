...
func TestNewWalletLedgerEntry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "should accept when a debit matches the balance arithmetic", transactionID: domain.NewID(), direction: ledger.Debit,
			money: domaintest.MustParseMoney(t, "25.00", "BRL"), balanceBefore: before, balanceAfter: domaintest.MustParseMoney(t, "75.00", "BRL"),
		},
		{
			name: "should accept when a credit matches the balance arithmetic", transactionID: domain.NewID(), direction: ledger.Credit,
			money: domaintest.MustParseMoney(t, "25.00", "BRL"), balanceBefore: before, balanceAfter: domaintest.MustParseMoney(t, "125.00", "BRL"),
		},
		{
			name: "should return INVALID_AMOUNT when balanceAfter does not match the arithmetic", transactionID: domain.NewID(), direction: ledger.Debit,
		...
		{
			name: "should return INVALID_AMOUNT when the entry amount is zero", transactionID: domain.NewID(), direction: ledger.Credit,
		...
		{
			name: "should return CURRENCY_MISMATCH when the amount is in another currency", transactionID: domain.NewID(), direction: ledger.Credit,
		...
		{
			name: "should return INVALID_INPUT when the transaction id is nil", transactionID: domain.NilID, direction: ledger.Credit,
			money: domaintest.MustParseMoney(t, "1.00", "BRL"), balanceBefore: before, balanceAfter: domaintest.MustParseMoney(t, "101.00", "BRL"),
			wantErr: domain.FailureCodeInvalidInput,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			entry, err := ledger.NewEntry(domain.NewID(), domain.NewID(), domain.NewID(),
				test.direction, amount, before, test.balanceAfter)
