...
// FixedNow is the instant the fixtures stamp on everything they build, so that a
// test asserts timestamps without reading the clock.
...
func MustOpenWallet(t testing.TB, initialBalance money.Money) *wallet.Wallet {
	...
	opened, err := wallet.Open(domain.NewID(), domain.NewID(), initialBalance, FixedNow)
	require.NoError(t, err)
	return opened
}
...
func MustProcessedReference(t testing.TB, w *wallet.Wallet, kind wager.Kind, amount string) *wager.Transaction {
	...
	require.NoError(t, err)
	require.NoError(t, reference.MarkProcessed(w.Balance(), FixedNow))
	return reference
}
...
func MustRehydrateWallet(t testing.TB, id, playerID domain.ID, balance string, version int64) *wallet.Wallet {
	...
	rehydrated, err := wallet.Rehydrate(id, playerID, MustParseMoney(t, balance, "BRL"), version, FixedNow, FixedNow)
	require.NoError(t, err)
	return rehydrated
}

// MustRehydrateTransaction rebuilds an operation with the given state, failing
// the test otherwise. Timestamps left empty are stamped with FixedNow, so a
// table states only what its case is about.
func MustRehydrateTransaction(t testing.TB, params wager.RehydrateParams) *wager.Transaction {
	t.Helper()

	if params.CreatedAt.IsZero() {
		params.CreatedAt = FixedNow
	}
	if params.UpdatedAt.IsZero() {
		params.UpdatedAt = FixedNow
	}
